...
class ProdutoServico(models.Model):
    ...
    preco_unitario = models.DecimalField(
        max_digits=8, decimal_places=2, default=0, verbose_name='Preço Unitário')
...
class Venda(models.Model):
    data_criacao = models.DateTimeField(
        auto_now=True, verbose_name='Data criação')
    vendedor = models.ForeignKey(
        Vendedor, blank=True, null=True, verbose_name='Vendedor', on_delete=models.PROTECT)
    cliente = models.ForeignKey(
        Cliente, verbose_name='Cliente', on_delete=models.PROTECT)
    situacao = models.IntegerField(
        choices=SITUACAO_VENDA_CHOICES, verbose_name='Situação', default=1)
    valor_total = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name='Valor Total Venda')
    valor_comissao_total = models.DecimalField(
        max_digits=8, decimal_places=2, default=0, verbose_name='Valor Total de Comissão')

    def calcula_valor_total(self):
        list_total = self.itemvenda_set.filter(
            venda=self).values_list('total', flat=True)
        total = sum(list_total)
        self.valor_total = total

    def calcula_valor_comissao_total(self):
        list_comissao_total = self.itemvenda_set.filter(
            venda=self).values_list('total_comissao', flat=True)
...
class ItemVenda(models.Model):
    ...
    total_comissao = models.DecimalField(
        max_digits=8, decimal_places=2, default=0, verbose_name='Total de Comissão')
